...
- `vk_market_retry_manifest.csv` — список ID, собранных для повторного прогона.
...
Если отдельные партии были пропущены после ошибок VK, дождитесь завершения
основной очереди и запустите отдельный повторный прогон:

```bash
./retry_market_errors.sh
```

Он читает `vk_market_skipped_batches.jsonl`, повторяет только товары из
проблемных партий и делит их на пачки по 10. Товары, которые успели добавиться
из частично успешной партии, VK повторно не создаёт: YML использует стабильный
ID из Excel.

Если VK оставляет окно импорта без движения, worker через 10 минут считает
попытку зависшей. Проблемная партия сразу попадает в журнал без циклических
повторов, а очередь продолжает работу.

Если сам раздел импорта VK отвечает `Access denied` или временно не
открывается, текущая партия не считается ошибочной. Фоновый worker проверяет
интерфейс раз в 5 минут и автоматически продолжает очередь после его
восстановления. Это защищает все последующие товары от ложного пропуска.
